...
type Ink = { x: number; y: number; w: number; h: number; color: string; weight: number };
...
function makeParticle(x: number, y: number, color: string, W: number, H: number): DustParticle {
  ...
  return {
    ...
    x,
    y,
    size: isChar ? 10 + Math.random() * 3 : 1.4 + Math.random() * 2.4,
    vx: (Math.random() - 0.5) * 2.2,
    lift: 0.7 + Math.random() * 2,
    wobble: Math.random() * Math.PI * 2,
    color,
    opacity: 0.75 + Math.random() * 0.25,
    delay: clamp01(sweepAt(x, y, W, H) + (Math.random() - 0.5) * 0.1),
...
export function sampleViewport(): DustSample {
  ...
  if (!shell) return { particles: [], pieces: [], W, H };
  ...
  return { particles: distribute(collectInk(pieces, W, H, styleOf), W, H), pieces, W, H };
}
 ...
 */
export function markPieces(pieces: HTMLElement[], sweepMs: number, W: number, H: number) {
  const delays = pieces.map((el) => {
    const r = el.getBoundingClientRect();
    // 35% dentro da caixa: a peça começa a sumir quando a frente do varrimento a alcança.
    return sweepAt(r.left + r.width * 0.35, r.top + r.height * 0.35, W, H);
  });
